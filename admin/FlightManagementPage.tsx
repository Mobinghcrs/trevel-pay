import React, { useState, useEffect, useCallback } from 'react';
import { Flight } from '../types';
import { getAdminFlights, createAdminFlight, updateAdminFlight, deleteAdminFlight } from '../services/apiService';
import Spinner from '../components/Spinner';
import { ICONS } from '../constants';
import FlightTableRow from './components/FlightTableRow';
import FlightFormModal from './components/FlightFormModal';

const FlightManagementPage: React.FC = () => {
    const [flights, setFlights] = useState<Flight[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingFlight, setEditingFlight] = useState<Flight | null>(null);

    const fetchFlights = useCallback(async () => {
        setIsLoading(true);
        setError(null);
        try {
            const data = await getAdminFlights();
            setFlights(data);
        } catch (err) {
            console.error(err);
            setError(err instanceof Error ? err.message : "Failed to load flights.");
        } finally {
            setIsLoading(false);
        }
    }, []);

    useEffect(() => {
        fetchFlights();
    }, [fetchFlights]);
    
    const handleOpenModal = (flight: Flight | null = null) => {
        setEditingFlight(flight);
        setIsModalOpen(true);
    };

    const handleCloseModal = () => {
        setIsModalOpen(false);
        setEditingFlight(null);
    };

    const handleSaveFlight = async (flightData: Omit<Flight, 'id'> | Flight) => {
        try {
            if ('id' in flightData && flightData.id) {
                // Editing existing flight
                const { id, ...updateData } = flightData;
                await updateAdminFlight(id, updateData);
            } else {
                // Creating new flight
                await createAdminFlight(flightData);
            }
            fetchFlights(); // Refresh data
            handleCloseModal();
        } catch (err) {
            alert(`Error saving flight: ${err instanceof Error ? err.message : 'Unknown error'}`);
        }
    };
    
    const handleDeleteFlight = async (flightId: string) => {
        if (confirm('Are you sure you want to permanently delete this flight?')) {
            try {
                await deleteAdminFlight(flightId);
                fetchFlights(); // Refresh data
            } catch (err) {
                 alert(`Error deleting flight: ${err instanceof Error ? err.message : 'Unknown error'}`);
            }
        }
    };


    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-3xl font-bold text-slate-900">Flight Management</h1>
                <button 
                    onClick={() => handleOpenModal()}
                    className="flex items-center gap-2 bg-cyan-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-cyan-600 transition-colors"
                >
                    <span className="h-5 w-5">{ICONS.add}</span>
                    <span>Add New Flight</span>
                </button>
            </div>

            {error && <div className="text-center text-red-600 bg-red-100 p-4 rounded-md mb-4">{error}</div>}
            
            <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                            <tr>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Flight</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Route</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Schedule</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Price</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-slate-200">
                            {isLoading ? (
                                <tr><td colSpan={5}><Spinner message="Loading flights..." /></td></tr>
                            ) : flights.length === 0 ? (
                                <tr><td colSpan={5} className="text-center p-8 text-slate-500">No flights defined. Add one to get started.</td></tr>
                            ) : (
                                flights.map(flight => (
                                    <FlightTableRow 
                                        key={flight.id} 
                                        flight={flight} 
                                        onEdit={() => handleOpenModal(flight)}
                                        onDelete={() => handleDeleteFlight(flight.id)}
                                    />
                                ))
                            )}
                        </tbody>
                    </table>
                </div>
            </div>

            {isModalOpen && (
                 <FlightFormModal
                    isOpen={isModalOpen}
                    onClose={handleCloseModal}
                    onSave={handleSaveFlight}
                    flight={editingFlight}
                />
            )}
        </div>
    );
};

export default FlightManagementPage;
