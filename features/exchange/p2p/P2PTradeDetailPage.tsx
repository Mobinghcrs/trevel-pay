            ...
            <div className="p-6 space-y-6">
                ...
                <div className="bg-slate-100 p-4 rounded-lg space-y-3">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-mono text-slate-800 font-bold text-lg">{offer.amountAvailable.toFixed(2)} {offer.currency}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="text-slate-600">Price per {offer.currency}</span>
                        <span className="font-mono text-slate-800">{offer.pricePerUnit.toFixed(2)} LCU</span>
                    </div>
                    <div className="flex justify-between items-center text-lg border-t border-slate-200 pt-3">
                        ...
                        <span className="font-mono font-bold text-teal-600">{total.toFixed(2)} LCU</span>
                    </div>
                </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Confirm Transaction">
          <div className="space-y-4 text-slate-800">
              ...
              <div className="bg-slate-100 p-4 rounded-lg space-y-2">
                  ...
                  <div className="flex justify-between"><span className="text-slate-600">Amount:</span> <span className="font-mono font-semibold text-slate-800">{offer.amountAvailable.toFixed(2)} {offer.currency}</span></div>
                  <div className="flex justify-between"><span className="text-slate-600">Total Price:</span> <span className="font-mono font-semibold text-slate-800">{total.toFixed(2)} LCU</span></div>
              </div>
