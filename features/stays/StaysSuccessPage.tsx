...
interface StaysSuccessPageProps {
...
const StaysSuccessPage: React.FC<StaysSuccessPageProps> = ({ order, onNewBooking }) => {
    const formatDate = (dateString: string) => new Date(dateString).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
    const qrValue = JSON.stringify({ bookingId: order.id, guest: order.guests[0].fullName, stay: order.stay.name });
    ...
    const handleDownloadPDF = () => {
        const doc = new jsPDF();
        doc.setFontSize(22);
        doc.text("Stay Booking Voucher", 14, 22);
        doc.setFontSize(12);
        ...
        autoTable(doc, {
            ...
            head: [['Item', 'Details']],
            body: [
                ['Property', `${order.stay.name} (${order.stay.type})`],
                ['Location', order.stay.location],
                ['Check-in', formatDate(order.checkInDate)],
                ['Check-out', `${formatDate(order.checkOutDate)} (${nights} nights)`],
                ['Total Paid', `$${order.totalPrice.toLocaleString('en-US')}`],
            ],
            theme: 'striped',
        });
        
        doc.save(`TravelPay_Stay_Voucher_${order.id.slice(-6)}.pdf`);
    };
  ...
  return (
    <div className="max-w-lg mx-auto">
        <div className="text-center mb-6">
            ...
            <p className="text-slate-600 mt-2">Your accommodation voucher is ready. Please present it at check-in.</p>
        </div>
        ...
        <Card className="p-6 md:p-8 bg-white border border-slate-200">
            <div className="text-center mb-4">
                ...
                <h2 className="font-bold text-slate-800 text-lg">{order.stay.name}</h2>
...
export default StaysSuccessPage;