...
import { Hotel, Room, HotelGuest } from '../../types';
import Card from '../../components/Card';
import HotelBookingBreadcrumbs from './HotelBookingBreadcrumbs';

interface HotelGuestInfoPageProps {
  hotel: Hotel;
  room: Room;
...
const HotelGuestInfoPage: React.FC<HotelGuestInfoPageProps> = ({ hotel, room, onSubmit, onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // For simplicity, we'll only handle one guest's info for the booking.
    onSubmit([guest]);
  };

  return (
    <div>
      <HotelBookingBreadcrumbs currentStep="guests" />
        ...
        <div className="lg:col-span-1">
            <Card className="p-4 bg-slate-50 border-slate-200 sticky top-24">
                 ...
                 <div className="space-y-3 text-sm">
                    <img src={hotel.imageUrl} alt={hotel.name} className="rounded-lg mb-3" />
                    <p className="font-bold text-base text-slate-900">{hotel.name}</p>
                    <div className="flex justify-between">
                        <span className="text-slate-600">Room</span>
                        <span className="text-slate-900 font-semibold">{room.name}</span>
                    </div>
                    <div className="flex justify-between">
...
export default HotelGuestInfoPage;